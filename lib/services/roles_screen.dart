...
appBar: AppBar(
        ...
        children: [
          ...
          if (_usuarioActual?.tienePermiso(Permiso.controlTotal) ==
            ...
            IconButton(
              ...
              tooltip: 'Eliminar del sistema (solo Super Admin)',
              onPressed: () => _confirmarEliminarPersona(u),
            ),
        ],
      ),
    );
  ...
  void _confirmarEliminarPersona(UsuarioIglesia u) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          '¿Seguro que quieres eliminar a ${u.nombreCompleto} '
          'de esta iglesia? Perderá el acceso por completo y '
          'tendría que unirse de nuevo con un código de invitación.',
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(ctx);
              final error = await _rolesService.eliminarUsuarioDeIglesia(
                iglesiaId: _iglesiaId!,
                usuarioId: u.uid,
                quienElimina: _usuarioActual!,
              );
              if (!mounted) return;
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(error ??
                      '${u.nombreCompleto} fue eliminado de la iglesia'),
                  backgroundColor:
                      error == null ? AppColors.exito : AppColors.error,
